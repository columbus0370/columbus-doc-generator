...
export default function StepLineItems({ question, value, onChange, onNext, onBack, stepIndex, totalSteps }) {
          ...
          {rows.map((row, i) => (
            <div key={i} className="bg-navy-900/60 border border-navy-700 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">摘要（任意）</label>
                  <input
                    type="text"
                    value={row.desc}
